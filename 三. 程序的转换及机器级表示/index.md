...
> 教材：《计算机系统原理（2023年版）》，袁春风编著  
> 对应PDF第97～165页，教材页码90～158。  
> 本讲义严格保留原章、节、目层级，在此基础上重新组织解释、例子和图示，不是教材原文转录。  
> 汇编示例主要采用教材中的AT&T格式：源操作数在前、目的操作数在后；寄存器前加`%`，立即数前加`$`。
...
教材使用寄存器传送语言RTL描述指令功能。本讲义采用如下简化记号：
...
```asm
leal 3(%eax,%eax,4), %edx   # edx = 5*eax+3
```
